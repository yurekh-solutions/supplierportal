// Product placeholder images using placeholder service
const getProductImage = (category: string, productName: string) => {
  // Use a placeholder image service with category-based colors
  const categoryColors: Record<string, string> = {
    'mild-steel': '64748b',     // Slate
    'stainless-steel': '94a3b8', // Light slate
...
export const products: Product[] = [
  // Featured Priority Products (7 products)
  {
    ...
    image: getProductImage("mild-steel", "TMT Bars Fe 500D"),
  ...
  },
  {
    ...
    description: "Premium mild steel hollow square and rectangular sections for structural applications. Manufactured to IS 4923 standards with uniform wall thickness and precise dimensions.",
     param($match) 
  $lines = (Get-Content data.ts)
  $idx = [array]::IndexOf($lines, $match.Value.Trim())
  $catLine = $lines[($idx-2)]
  $nameLine = $lines[($idx-3)]
  $cat = if ($catLine -match 'category: "([^"]+)"') { $matches[1] } else { '' }
  $name = if ($nameLine -match 'name: "([^"]+)"') { $matches[1] } else { '' }
  "image: getProductImage(`"$cat`", `"$name`"),"
